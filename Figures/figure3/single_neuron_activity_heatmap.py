...
for i in range(all_peth.shape[0]):
    ...
    #norm_peth[i, :] = all_peth[i, :] - np.mean(all_peth[i, time_ax < 0])
...
img = ax_mb.imshow(these_peths, cmap=cmap,
                   vmin=VMIN, vmax=VMAX, extent=[-T_BEFORE, T_AFTER, -1.25, 1], interpolation='none')
ax_mb.add_patch(Rectangle((0, -1.25), 1, 2.5, color='royalblue', alpha=0.25, lw=0))
...
img = ax_fc.imshow(these_peths, cmap=cmap,
                 vmin=VMIN, vmax=VMAX, extent=[-T_BEFORE, T_AFTER, -1.25, 1], interpolation='none')
ax_fc.add_patch(Rectangle((0, -1.25), 1, 2.5, color='royalblue', alpha=0.25, lw=0))
...
cb_ax = f.add_axes([0.85, 0.1, 0.01, 0.35])
cbar = f.colorbar(mappable=ax_mb.images[0], cax=cb_ax)
cbar.ax.set_ylabel('Firing rate / baseline', rotation=270, labelpad=10)
