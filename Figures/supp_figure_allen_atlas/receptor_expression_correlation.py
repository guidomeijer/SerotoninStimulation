...
ephys_summary = ephys_data[['region', 'mod_index']].groupby('region').mean().reset_index()

# Drop some regions
ephys_summary = ephys_summary[ephys_summary['region'] != 'AI']
ephys_summary = ephys_summary[ephys_summary['region'] != 'ZI']
ephys_summary = ephys_summary[ephys_summary['region'] != 'root']
...
r, p = pearsonr(plot_data['mod_index'], plot_data['expression_energy'])
print(f'5-HT2a receptor; p = {np.round(p, 2)}')
...
for _, row in plot_data.iterrows():
    ax.scatter(row['mod_index'], row['expression_energy'], color=row['color'], s=20, zorder=1)
ax.set(xlabel='5-HT modulation index', ylabel='5-HT2A receptor expression', title='5-HT2A',
       xlim=[-0.2, 0.2], xticks=[-0.2, 0, 0.2], xticklabels=[-0.2, 0, 0.2],
       ylim=[0, 6], yticks=np.arange(7))
ax.legend(labels=plot_data['region'], bbox_to_anchor=(1.05, 1.1))
plt.tight_layout()
sns.despine(trim=True)
...
r, p = pearsonr(plot_data['mod_index'], plot_data['expression_energy'])
print(f'5-HT2C receptor; p = {np.round(p, 2)}')
...
for _, row in plot_data.iterrows():
    ax.scatter(row['mod_index'], row['expression_energy'], color=row['color'], s=20, zorder=1)
ax.set(xlabel='5-HT modulation index', ylabel='5-HT2C receptor expression', title='5-HT2C',
       xlim=[-0.2, 0.2], xticks=[-0.2, 0, 0.2], xticklabels=[-0.2, 0, 0.2],
       ylim=[0, 6], yticks=np.arange(7))
ax.legend(labels=plot_data['region'], bbox_to_anchor=(1.05, 1.1))
plt.tight_layout()
...
r, p = pearsonr(plot_data['mod_index'], plot_data['expression_energy'])
print(f'5-HT3A receptor; p = {np.round(p, 2)}')

slope, intercept = np.polyfit(plot_data['mod_index'], plot_data['expression_energy'], 1)
x_fit = np.linspace(-0.19, 0.19, 100)
y_fit = slope * x_fit + intercept
...
for _, row in plot_data.iterrows():
    ax.scatter(row['mod_index'], row['expression_energy'], color=row['color'], s=20, zorder=1)
ax.text(0, 0.8, f'p={np.round(p, 2)}', ha='center', va='center')
ax.set(xlabel='5-HT modulation index', ylabel='5-HT3A receptor expression', title='5-HT3A',
       xlim=[-0.2, 0.2], xticks=[-0.2, 0, 0.2], xticklabels=[-0.2, 0, 0.2],
       ylim=[0, 1.1], yticks=np.arange(1.1, step=0.2))
ax.legend(labels=plot_data['region'], bbox_to_anchor=(1.05, 1.1))
plt.tight_layout()
sns.despine(trim=True)
