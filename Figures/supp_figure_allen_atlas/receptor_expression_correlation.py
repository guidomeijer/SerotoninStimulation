...
for receptor in np.unique(expression_mean['receptor']):
    ...
    fig, axs = plt.subplots(1, 3, figsize=(1.75 * 3, 1.75), dpi=dpi, sharey=True)
    ...
    axs[0].set(xlabel='5-HT modulation index', ylabel=f'{receptor} receptor expression',
           xlim=[-0.2, 0.2], xticks=[-0.2, -0.1, 0, 0.1, 0.2], xticklabels=[-0.2, -0.1, 0, 0.1, 0.2],
           ylim=[use_min, use_max], yticks=yticks, title=f'p={np.round(p, 2)}')
    ...
    axs[1].set(xlabel='5-HT modulated neurons (%)', title=f'p={np.round(p, 2)}',
    ...
    axs[2].set(xlabel='Modulation latency (s)', title=f'p={np.round(p, 2)}',
