...
ephys_summary = ephys_summary[~np.isin(ephys_summary['region'], ['AI', 'ZI', 'root'])]
...
for _, row in plot_data.iterrows():
    ax.scatter(row['mod_index'], row['projection_density'], color=row['color'], s=20, zorder=1)
ax.set(xlabel='5-HT modulation index', ylabel='Dorsal raphe projection density', 
...
plt.savefig(join(fig_path, 'dr_projection_vs_modulation.pdf'))
...
slope, intercept = np.polyfit(plot_data['perc_mod'], plot_data['projection_density'], 1)
x_fit = np.linspace(-0.3, 0.3, 100)
y_fit = slope * x_fit + intercept

fig, ax = plt.subplots(figsize=(3.1, 2.3), dpi=dpi)
for _, row in plot_data.iterrows():
    ax.scatter(row['perc_mod'], row['projection_density'], color=row['color'], s=20, zorder=1)
ax.set(xlabel='5-HT modulated neurons (%)', ylabel='Dorsal raphe projection density', 
       xlim=[10, 40], xticks=np.arange(10, 45, step=10),
       ylim=[-0.005, 0.08], yticks=[0, 0.02, 0.04, 0.06, 0.08], yticklabels=[0, 0.02, 0.04, 0.06, 0.08])
ax.legend(labels=plot_data['region'], bbox_to_anchor=(1.05, 1.1))
plt.tight_layout()
sns.despine(trim=True)
