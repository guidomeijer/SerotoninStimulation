...
perc_ns['mod_index'] = ns_df.groupby(['subject']).mean(numeric_only=True)['mod_index_late']
perc_ns['type'] = 'NS'
...
perc_ws['mod_index'] = ws_df.groupby(['subject']).mean(numeric_only=True)['mod_index_late']
perc_ws['type'] = 'WS'
...
for i, subject in enumerate(np.unique(perc_mod_df['subject'])):
    ns = perc_mod_df.loc[(perc_mod_df['subject'] == subject) & (perc_mod_df['type'] == 'NS'), 'perc_mod']
    ws = perc_mod_df.loc[(perc_mod_df['subject'] == subject) & (perc_mod_df['type'] == 'WS'), 'perc_mod']
    ax.plot([0, 1], [ns, ws], color='grey', lw=1)
    ax.plot(0, ns, marker='o', ms=4, markeredgewidth=0.4, markeredgecolor='w', markerfacecolor=colors['NS'])
    ax.plot(1, ws, marker='o', ms=4, markeredgewidth=0.4, markeredgecolor='w', markerfacecolor=colors['WS'])
