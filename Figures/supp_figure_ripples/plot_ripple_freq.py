...
dg_spike_df = pd.read_csv(join(save_path, 'dg_spike_freq.csv'))
...
dg_spike_df = dg_spike_df[~dg_spike_df['pid'].isin(excl_pids)]

# Add expression
subjects = load_subjects()
for i, nickname in enumerate(np.unique(subjects['subject'])):
    ripple_df.loc[ripple_df['subject'] == nickname, 'sert-cre'] = subjects.loc[subjects['subject'] == nickname, 'sert-cre'].values[0]
    dg_spike_df.loc[dg_spike_df['subject'] == nickname, 'sert-cre'] = subjects.loc[subjects['subject'] == nickname, 'sert-cre'].values[0]
ripple_df = ripple_df[ripple_df['sert-cre'] == 1]
dg_spike_df = dg_spike_df[dg_spike_df['sert-cre'] == 1]
...
#ax2.add_patch(Rectangle((0, 0), 1, 0.35, color='royalblue', alpha=0.25, lw=0))
sns.lineplot(data=dg_spike_df, x='time', y='dg_spike_freq', errorbar='se', color='k',
             err_kws={'lw': 0}, ax=ax2)
ax2.text(2, 0.32, f'n = {np.unique(dg_spike_df["subject"]).shape[0]} mice', ha='left', va='center')
ax2.set(xlabel='Time from stimulation onset (s)',
        xticks=[-1, 0, 1, 2, 3, 4])
ax2.set_ylabel('DG spikes per second', labelpad=-5)
