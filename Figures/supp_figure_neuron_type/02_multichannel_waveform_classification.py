...
WS1_label = left_15_df.groupby('group_label').mean(numeric_only=True)['v_below'].idxmax()
types = np.array([0, 1, 2])
WS2_label = types[~np.isin(types, np.array([fs_label, WS1_label]))][0]
if WS2_label == fs_label:
    WS2_label = types[~np.isin(types, np.array([fs_label, WS1_label]))][0]
left_15_df.loc[left_15_df['group_label'] == WS1_label, 'type'] = 'WS1'
left_15_df.loc[left_15_df['group_label'] == WS2_label, 'type'] = 'WS2'

# Print result
FS_perc = (np.sum(left_15_df['type'] == 'NS') / left_15_df.shape[0]) * 100
WS1_perc = (np.sum(left_15_df['type'] == 'WS1') / left_15_df.shape[0]) * 100
WS2_perc = (np.sum(left_15_df['type'] == 'WS2') / left_15_df.shape[0]) * 100
print(f'\nLeft 15 deg\nFS: {FS_perc:.2f}%\nWS1: {WS1_perc:.2f}%\nWS2: {WS2_perc:.2f}%')
...
WS1_label = right_10_df.groupby('group_label').mean(numeric_only=True)['v_below'].idxmax()
types = np.array([0, 1, 2])
WS2_label = types[~np.isin(types, np.array([fs_label, WS1_label]))][0]
if WS2_label == fs_label:
    WS2_label = types[~np.isin(types, np.array([fs_label, WS1_label]))][0]
right_10_df.loc[right_10_df['group_label'] == WS1_label, 'type'] = 'WS1'
right_10_df.loc[right_10_df['group_label'] == WS2_label, 'type'] = 'WS2'

# Print result
FS_perc = (np.sum(right_10_df['type'] == 'NS') / right_10_df.shape[0]) * 100
WS1_perc = (np.sum(right_10_df['type'] == 'WS1') / right_10_df.shape[0]) * 100
WS2_perc = (np.sum(right_10_df['type'] == 'WS2') / right_10_df.shape[0]) * 100
print(f'\nRight 10 deg\nFS: {FS_perc:.2f}%\nWS1: {WS1_perc:.2f}%\nWS2: {WS2_perc:.2f}%')
...
WS1_label = left_10_df.groupby('group_label').mean(numeric_only=True)['v_below'].idxmax()
types = np.array([0, 1, 2])
WS2_label = types[~np.isin(types, np.array([fs_label, WS1_label]))][0]
if WS2_label == fs_label:
    WS2_label = types[~np.isin(types, np.array([fs_label, WS1_label]))][0]
left_10_df.loc[left_10_df['group_label'] == WS1_label, 'type'] = 'WS1'
left_10_df.loc[left_10_df['group_label'] == WS2_label, 'type'] = 'WS2'

# Print result
FS_perc = (np.sum(left_10_df['type'] == 'NS') / left_10_df.shape[0]) * 100
WS1_perc = (np.sum(left_10_df['type'] == 'WS1') / left_10_df.shape[0]) * 100
WS2_perc = (np.sum(left_10_df['type'] == 'WS2') / left_10_df.shape[0]) * 100
print(f'\nLeft 10 deg\nFS: {FS_perc:.2f}%\nWS1: {WS1_perc:.2f}%\nWS2: {WS2_perc:.2f}%')
...
# Then do another clustering on the RS group to split those into WS1 and WS2
...
# Get the WS1 and WS2 labels right
if (waveforms_rs_df.loc[waveforms_rs_df['group_label'] == 0, 'v_below'].mean()
    ...
    # type 0 is WS1
    waveforms_rs_df.loc[waveforms_rs_df['group_label'] == 0, 'type'] = 'WS1'
    waveforms_rs_df.loc[waveforms_rs_df['group_label'] == 1, 'type'] = 'WS2'
else:
    # type 1 is narrow spiking
    waveforms_rs_df.loc[waveforms_rs_df['group_label'] == 0, 'type'] = 'WS2'
    waveforms_rs_df.loc[waveforms_rs_df['group_label'] == 1, 'type'] = 'WS1'

# Merge into original dataframe
waveforms_df.loc[waveforms_rs_df.index.values, 'type'] = waveforms_rs_df['type']

# Assume neurons with v_below of 0 to be WS1
waveforms_df.loc[(waveforms_df['v_below'] == 0) & (waveforms_df['type'] == 'RS'), 'type'] = 'WS1'
...
WS1_perc = (np.sum(waveforms_df['type'] == 'WS1') / waveforms_df[waveforms_df['type'] != 'Und.'].shape[0]) * 100
WS2_perc = (np.sum(waveforms_df['type'] == 'WS2') / waveforms_df[waveforms_df['type'] != 'Und.'].shape[0]) * 100
print(f'\nOverall result\nFS: {FS_perc:.2f}%\nWS1: {WS1_perc:.2f}%\nWS2: {WS2_perc:.2f}%')
...
ax1.plot(time_ax, waveforms_df.loc[waveforms_df['type'] == 'WS1', 'waveform'].to_numpy().mean(),
         color=colors['WS1'], label='WS1')
ax1.plot(time_ax, waveforms_df.loc[waveforms_df['type'] == 'WS2', 'waveform'].to_numpy().mean(),
         color=colors['WS2'], label='WS2')
...
ax2.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS1', 'spike_width'],
            waveforms_df.loc[waveforms_df['type'] == 'WS1', 'pt_ratio'], label='WS1', color=colors['WS'], s=1)
ax2.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS2', 'spike_width'],
            waveforms_df.loc[waveforms_df['type'] == 'WS2', 'pt_ratio'], label='WS2', color=colors['WS'], s=1)
...
ax3.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS1', 'spike_width'],
            waveforms_df.loc[waveforms_df['type'] == 'WS1', 'firing_rate'], label='WS1', color=colors['WS'], s=1)
ax3.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS2', 'spike_width'],
            waveforms_df.loc[waveforms_df['type'] == 'WS2', 'firing_rate'], label='WS2', color=colors['WS'], s=1)
...
ax4.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS1', 'spike_width'],
            waveforms_df.loc[waveforms_df['type'] == 'WS1', 'spread'], label='WS1', color=colors['WS1'], s=1)
ax4.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS2', 'spike_width'],
            waveforms_df.loc[waveforms_df['type'] == 'WS2', 'spread'], label='WS2', color=colors['WS2'], s=1)
...
ax5.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS1', 'v_below'],
            waveforms_df.loc[waveforms_df['type'] == 'WS1', 'spread'], label='WS1', color=colors['WS1'], s=1)
ax5.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS2', 'v_below'],
            waveforms_df.loc[waveforms_df['type'] == 'WS2', 'spread'], label='WS2', color=colors['WS2'], s=1)
#ax5.scatter(waveforms_df.loc[waveforms_df['type'] == 'NS', 'v_below'],
#            waveforms_df.loc[waveforms_df['type'] == 'NS', 'spread'], label='NS', color=colors['NS'], s=1)
ax5.set(xlabel='Velocity below (ms/mm)', ylabel='Waveform spread (mm)')

ax6.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS1', 'v_below'],
            waveforms_df.loc[waveforms_df['type'] == 'WS1', 'upper_lim'], label='WS1', color=colors['WS1'], s=1)
ax6.scatter(waveforms_df.loc[waveforms_df['type'] == 'WS2', 'v_below'],
            waveforms_df.loc[waveforms_df['type'] == 'WS2', 'upper_lim'], label='WS2', color=colors['WS2'], s=1)
...
for i in waveforms_df.loc[waveforms_df['type'] == 'WS1'].index:
...
for i in waveforms_df.loc[waveforms_df['type'] == 'WS2'].index:
...
ax2.imshow(np.flipud(waveforms_2), cmap='coolwarm', aspect='auto',
           vmin=-0.07, vmax=0.07)
ax2.text(30.5, 1.5, 'WS1', fontsize=7, color='k')
ax2.text(53, 10, f'n={np.sum(waveforms_df["type"] == "WS1")}', fontsize=5, color='k')
...
ax3.imshow(np.flipud(waveforms_3), cmap='coolwarm', aspect='auto',
           vmin=-0.07, vmax=0.07)
ax3.text(30.5, 1.5, 'WS2', fontsize=7, color='k')
ax3.text(53, 10, f'n={np.sum(waveforms_df["type"] == "WS2")}', fontsize=5, color='k')
...
sns.lineplot(data=dist_time_type_df[dist_time_type_df['type'] != 'Und.'], x='dist_soma', y='time_soma',
             ax=ax1, errorbar='se', hue='type', hue_order=['NS', 'WS1', 'WS2'],
             palette=[colors['NS'], colors['WS1'], colors['WS2']])
...
if waveforms_df.loc[waveforms_df['type'] == 'WS1'].shape[0] > 0:
    for i in waveforms_df.loc[waveforms_df['type'] == 'WS1'].index:
