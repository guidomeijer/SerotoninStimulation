...
fig_path = join(fig_path, 'Extra plots', 'Sniffing')
...
for k, nickname in enumerate(np.unique(rec['subject'])):
    ...
    for i, eid in enumerate(eids):
        ...
        elif sniffing.shape[0] > video_times.shape[0]:
            sniffing = sniffing[:video_times.shape[0]]

        # Calculate percentage change
        sniffing_perc = ((sniffing - np.percentile(sniffing[~np.isnan(sniffing)], 2))
                         / np.percentile(sniffing[~np.isnan(sniffing)], 2)) * 100

        # Get trial triggered baseline subtracted pupil sniffing
        for t, trial_start in enumerate(opto_train_times):
            this_sniffing = np.array([np.nan] * TIME_BINS.shape[0])
            baseline_subtracted = np.array([np.nan] * TIME_BINS.shape[0])
            baseline = np.nanmean(sniffing_perc[(video_times > (trial_start - BASELINE[0]))
                                                  & (video_times < (trial_start - BASELINE[1]))])
            for b, time_bin in enumerate(TIME_BINS):
                this_sniffing[b] = np.nanmean(sniffing_perc[
                    (video_times > (trial_start + time_bin) - (BIN_SIZE / 2))
                    & (video_times < (trial_start + time_bin) + (BIN_SIZE / 2))])
                baseline_subtracted[b] = np.nanmean(sniffing_perc[
                    (video_times > (trial_start + time_bin) - (BIN_SIZE / 2))
                    & (video_times < (trial_start + time_bin) + (BIN_SIZE / 2))]) - baseline
            sniffing_ses = pd.concat((sniffing_ses, pd.DataFrame(data={
                'sniffing': this_sniffing, 'baseline_subtracted': baseline_subtracted, 'eid': eid,
        ...
        results_df = pd.concat((results_df, pd.DataFrame(data={
            'sniffing': sniffing_ses[sniffing_ses['eid'] == eid].groupby('time').mean(numeric_only=True)['sniffing'],
            'baseline_subtracted': sniffing_ses[sniffing_ses['eid'] == eid].groupby('time').mean(numeric_only=True)['baseline_subtracted'],
            'time': TIME_BINS, 'subject': nickname, 'expression': expression, 'eid': eid})), ignore_index=True)

    # Save output
    results_df.to_csv(join(save_path, 'sniffing_passive.csv'))

    # Plot this animal
    colors, dpi = figure_style()
    sniffing_ses = sniffing_ses.reset_index(drop=True)

    f, (ax1, ax2) = plt.subplots(1, 2, figsize=(4, 2), dpi=dpi)
    sns.lineplot(x='time', y='sniffing', estimator=np.nanmean, data=sniffing_ses,
    ...
    sns.lineplot(x='time', y='baseline_subtracted', estimator=np.nanmean, data=sniffing_ses,
    ...
    plt.savefig(join(fig_path, f'{nickname}_sniffing_passive.jpg'), dpi=600)
    plt.close(f)
