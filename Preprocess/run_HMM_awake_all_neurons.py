...
if OVERWRITE:
    ...
    state_trans_df = pd.read_csv(
        join(save_path, f'all_state_trans_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
    p_state_df = pd.read_csv(
        join(save_path, f'p_state_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
    p_state_null_df = pd.read_csv(
        join(save_path, f'p_state_null_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
    state_trans_null_df = pd.read_csv(
        join(save_path, f'all_state_trans_null_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
...
for i, eid in enumerate(np.unique(rec['eid'])):
    ...
    # Save output
    state_trans_df.to_csv(
        join(save_path, f'all_state_trans_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
    p_state_df.to_csv(
        join(save_path, f'p_state_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
    state_trans_null_df.to_csv(
        join(save_path, f'all_state_trans_null_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
    p_state_null_df.to_csv(
        join(save_path, f'p_state_null_all-neurons_{int(BIN_SIZE*1000)}msbins_{INCL_NEURONS}.csv'))
