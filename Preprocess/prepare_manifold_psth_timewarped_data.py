...
save_path = join(s_path, 'manifold', 'timewarped')
...
for i, pid in enumerate(np.unique(task_neurons['pid'])):
    ...
    # Get binned spike rate for warped time
    binned_rate, neuron_ids = binned_rate_timewarped(
    ...
    for split in SPLITS:
        ...
        if split == 'L_opto':
            peth_dict[split] = np.mean(binned_rate[
