...
cca_df = cca_df[cca_df['sert-cre'] == 1].reset_index()
...
for i in cca_df.index:
    axs[i].fill_between(cca_df.loc[i, 'time'], cca_df.loc[i, 'r_mean'] - cca_df.loc[i, 'r_std'],
                        cca_df.loc[i, 'r_mean'] + cca_df.loc[i, 'r_std'], alpha=0.2)
    axs[i].plot(cca_df.loc[i, 'time'], cca_df.loc[i, 'r_mean'])
    axs[i].set(ylabel='Canonical correlation (r)', xlabel='Time (s)', xticks=[-1, 0, 1, 2, 3],
