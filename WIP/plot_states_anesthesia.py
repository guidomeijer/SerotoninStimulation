...
for i, region in enumerate(regions):
    ...
    if i == 0:
        axs[i].set_ylabel('State probability (%)', labelpad=0)
        axs[i].get_xaxis().set_visible(False)
        sns.despine(trim=True, bottom=True, ax=axs[i])
        axs[i].plot([0, 2], [-0.01, -0.01], color='k', lw=0.5)
...
plt.subplots_adjust(left=0.05, bottom=0.15, right=1, top=0.85, wspace=0, hspace=0.4)
plt.savefig(join(fig_path, 'p_updown_state_change_anesthesia.jpg'), dpi=600)
...
plt.savefig(join(fig_path, 'p_all_state_change_anesthesia.jpg'), dpi=600)
"""
