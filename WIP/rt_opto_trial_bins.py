...
# Get paths
fig_path = join(paths()[0], 'Extra plots', 'Behavior')
...
for i, subject in enumerate(subjects['subject']):
    ...
    for s in np.unique(trials['session']):
        ...
        for b, trial_ind in enumerate(opto_probe_ind):
            ...
            this_probe_df = pd.concat((this_probe_df, pd.DataFrame(data={
                'rt': these_repeats, 'trial': these_trials,
    ...
    rep_contrast = (trials.groupby(['unsigned_contrast', 'laser_stimulation']).sum()['rt']
                    / trials.groupby(['unsigned_contrast', 'laser_stimulation']).size()).reset_index()
    rep_contrast = rep_contrast.rename(columns={0: 'rt'})
    rep_contrast['subject'] = subject
    rep_contrast['sert-cre'] = sert_cre    
            
    # Remove probe trials
    #trials.loc[(trials['laser_probability'] == 0.25) | (trials['laser_probability'] == 0.75), 'rt'] = np.nan
        ...
        for b, trial_ind in enumerate(opto_block_switch_ind):
            ...
            for tt, this_edge in enumerate(TRIAL_BINS[:-1]):
                if this_ses[trial_ind+this_edge:trial_ind+TRIAL_BINS[tt+1]].shape[0] == trial_bin_size:
                    these_repeats = this_ses.loc[trial_ind+this_edge:(trial_ind+TRIAL_BINS[tt+1])-1, 'rt'].values
                    these_p_rep[tt] = np.sum(these_repeats) / these_repeats.shape[0]
                        
            this_block_df = pd.concat((this_block_df, pd.DataFrame(data={
                'rt': these_p_rep, 'trial_bin': trial_bin_labels,
    ...
    # Add to dataframe
    """
    this_repeats = this_probe_df[this_probe_df['opto'] == 1].groupby('trial').mean()['rt'].values * 100
    p_repeat_probe_df = pd.concat((p_repeat_probe_df, pd.DataFrame(data={
        ...
        'opto': 1})))
    this_repeats = this_probe_df[this_probe_df['opto'] == 0].groupby('trial').mean()['rt'].values * 100
    p_repeat_probe_df = pd.concat((p_repeat_probe_df, pd.DataFrame(data={
        'p_repeat': this_repeats, 'p_repeat_bl': this_repeats - np.mean(this_repeats[:SINGLE_TRIALS[0]]),
        'trial': np.arange(-SINGLE_TRIALS[0], SINGLE_TRIALS[1]+1), 'sert-cre': sert_cre, 'subject': subject,
        'opto': 0})))
    
    this_repeats = this_block_single_df[this_block_single_df['opto'] == 1].groupby('trial').mean()['rt'].values * 100
    p_repeat_single_df = pd.concat((p_repeat_single_df, pd.DataFrame(data={
        'p_repeat': this_repeats, 'p_repeat_bl': this_repeats - np.mean(this_repeats[:SINGLE_TRIALS[0]]),
        'trial': np.arange(-SINGLE_TRIALS[0], SINGLE_TRIALS[1]+1), 'sert-cre': sert_cre, 'subject': subject,
        'opto': 1})))
    this_repeats = this_block_single_df[this_block_single_df['opto'] == 0].groupby('trial').mean()['rt'].values * 100
    p_repeat_single_df = pd.concat((p_repeat_single_df, pd.DataFrame(data={
        'p_repeat': this_repeats, 'p_repeat_bl': this_repeats - np.mean(this_repeats[:SINGLE_TRIALS[0]]),
        'trial': np.arange(-SINGLE_TRIALS[0], SINGLE_TRIALS[1]+1), 'sert-cre': sert_cre, 'subject': subject,
        'opto': 0})))
    """
    this_repeats = this_block_df[this_block_df['opto'] == 1].groupby('trial_ind').mean(numeric_only=True)['rt'] * 100
    p_repeat_bins_df = pd.concat((p_repeat_bins_df, pd.DataFrame(data={
        ...
        'sert-cre': sert_cre,
        'opto': 1})))
    this_repeats = this_block_df[this_block_df['opto'] == 0].groupby('trial_ind').mean(numeric_only=True)['rt'] * 100
    p_repeat_bins_df = pd.concat((p_repeat_bins_df, pd.DataFrame(data={
        'p_repeat': this_repeats,
        'p_repeat_bl': this_repeats - np.mean(this_repeats.values[:np.sum(trial_bin_labels < 0)]),
        'trial': trial_bin_labels, 'subject': subject,
...
for t, trial in enumerate(trial_bin_labels):
    ...
    p_block[t] = ttest_rel(
        p_repeat_bins_df.loc[(p_repeat_bins_df['trial'] == trial)
                             & (p_repeat_bins_df['sert-cre'] == 1),
                             'p_repeat_bl'],
        p_repeat_bins_df.loc[(p_repeat_bins_df['trial'] == -1.5)
                             & (p_repeat_bins_df['sert-cre'] == 1),
                             'p_repeat_bl'])[1]
    """
...
labels = ['No stim', 'Stim']
ax3.legend(handles, labels, frameon=False, prop={'size': 5}, loc='lower right')
ax3.set(ylabel='P[repeat choice] (%)', xlabel='Trials since start of stim. block',
        xticks=np.arange(-SINGLE_TRIALS[0], SINGLE_TRIALS[-1]+1, 2), yticks=[-10, -5, 0, 5, 10])
...
plt.tight_layout()
plt.savefig(join(fig_path, 'rt_opto.jpg'), dpi=600)
...
sns.lineplot(data=p_repeat_bins_df[p_repeat_bins_df['sert-cre'] == 1],
             x='trial', y='p_repeat_bl', errorbar='se', err_style='bars', hue='opto', ax=ax1)
#ax1.set(ylabel='P[repeat choice] (%)', xlabel='Trials since start of stim. block',
#        xticks=np.arange(TRIAL_BINS[0], TRIAL_BINS[-1]+1, 10),
#        yticks=np.arange(-2, 7, 2))
plt.tight_layout()
sns.despine(trim=True)
plt.savefig(join(fig_path, 'rt_opto_block.jpg'), dpi=600)
