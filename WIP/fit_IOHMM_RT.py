...
# Settings
SINGLE_TRIALS = [2, 10]
WIN_STARTS = np.arange(-15, 70) 
WIN_SIZE = 15
trial_win_labels = WIN_STARTS + (WIN_SIZE/2)

"""
TRIAL_BINS = np.arange(-15, 51, 5)
...
for i, subject in enumerate(subjects['subject']):
    ...
    block_df = pd.concat((block_df, pd.DataFrame(data={
        'p_state': this_state,
        'p_state_bl': this_state - np.mean(this_state.values[:np.sum(trial_win_labels < 0)]),
...
for i in state_df.index.values:
    ax1.plot([0, 1], [state_df.loc[i, 'eng_stim'], state_df.loc[i, 'eng_nostim']],
             marker='o', color='k')
    
f, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(3.5, 1.75), dpi=dpi)

sns.lineplot(data=block_df, x='trial', y='p_state_bl', hue='opto', ax=ax1,
             errorbar='se', marker='o')

sns.lineplot(data=block_single_df, x='trial', y='p_state_bl', hue='opto', ax=ax2,
             errorbar='se')

sns.lineplot(data=probe_df, x='trial', y='p_state_bl', hue='opto', ax=ax3,
             errorbar='se')
