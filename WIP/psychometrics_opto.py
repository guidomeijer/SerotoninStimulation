...
fig_path, _ = paths()
fig_path = join(fig_path, 'Extra plots', 'Behavior')
...
for i, nickname in enumerate(subjects['subject']):
    ...
    lapse_r_r_s = trials.loc[(trials['probabilityLeft'] == 0.2) & (trials['signed_contrast'] == 1)
                              & (trials['laser_probability'] == 1), 'correct'].mean()
    lapse_df = pd.concat((lapse_df, pd.DataFrame(data={
        'subject': nickname, 'sert-cre': subjects.loc[i, 'sert-cre'],
    ...
    if PLOT_SINGLE_ANIMALS:
        ...
        ax1.set_ylabel('Right choices (%)', labelpad=0)

        sns.despine(trim=True)
        
        plt.subplots_adjust(left=0.22, right=0.98, bottom=0.2, top=0.95)
...
for i, subject in enumerate(psy_avg_block_df['subject']):
    ax5.plot([1, 2], psy_avg_block_df.loc[(psy_avg_block_df['subject'] == subject), 'threshold'],
             color = colors[int(psy_avg_block_df.loc[psy_avg_block_df['subject'] == subject, 'expression'].unique()[1])],
             marker='o', ms=2)
ax5.set(xlabel='', xticks=[1, 2], xticklabels=['No stim', 'Stim'], ylabel='Threshold')
...
plt.tight_layout()
sns.despine(trim=True)
plt.savefig(join(fig_path, 'summary_psycurve_sert_wt.pdf'))

# %%
f, ((ax1, ax2, ax3, ax4), (ax5, ax6, ax7, ax8)) = plt.subplots(2, 4, figsize=(6, 3.5), dpi=dpi)
for i, subject in enumerate(bias_df['subject']):
    sert_cre = bias_df.loc[bias_df['subject'] == subject, 'sert-cre'].unique()[0]
    if sert_cre == 1:
        label = 'SERT'
        ...
        label = 'WT'
    ax1.plot([1, 2], [bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'bias_fit_no_stim'],
                      bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'bias_fit_stim']],
             color = colors[sert_cre], label=label, marker='o', ms=2)
ax1.set(xlabel='', xticks=[1, 2], xticklabels=['No stim.', 'Stim.'],
        ylabel='Bias', ylim=[0, 0.4], xlim=[0.8, 2.2])
#ax1.legend(frameon=False)


for i, subject in enumerate(bias_df['subject']):
    ax2.plot([1, 2], [bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'bias_probe_no_stim'],
                      bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'bias_probe_stim']],
             color = colors[bias_df.loc[bias_df['subject'] == subject, 'sert-cre'].values[0]], marker='o', ms=2)
ax2.set(xlabel='', xticks=[1, 2], xticklabels=['Non-stimulated\ntrials', 'Stimulated\ntrials'],
        ylabel='Total bias', ylim=[-0.1, 0.7],
        title='Probe trials')
...
for i, subject in enumerate(bias_df['subject']):
    ax3.plot([1, 2], [bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'side_bias_no_stim'],
                      bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'side_bias_stim']],
             color = colors[bias_df.loc[bias_df['subject'] == subject, 'sert-cre'].values[0]], marker='o', ms=2)
ax3.set(xlabel='', xticks=[1, 2], xticklabels=['Non-stimulated\ntrials', 'Stimulated\ntrials'],
        ylabel='Side bias', ylim=[-0.25, 0.25])

for i, subject in enumerate(bias_df.loc[bias_df['sert-cre'] == 1, 'subject']):
    ax4.plot([1, 2], [bias_df.loc[bias_df['subject'] == subject, 'side_bias_probe_no_stim'],
                      bias_df.loc[bias_df['subject'] == subject, 'side_bias_probe_stim']],
             color = colors[bias_df.loc[bias_df['subject'] == subject, 'sert-cre'].values[0]], marker='o', ms=2)
ax4.set(xlabel='', xticks=[1, 2], xticklabels=['Non-stimulated\ntrials', 'Stimulated\ntrials'],
...
for i, subject in enumerate(psy_avg_block_df[psy_avg_block_df['sert-cre'] == 1.0]['subject']):
    ax5.plot([1, 2], 
             ...
             color = colors[1],
             marker='o', ms=2)
ax5.set(xlabel='', xticks=[1, 2], xticklabels=['No stim.', 'Stim.'], ylabel='Threshold (σ)',
...
for i, subject in enumerate(bias_df['subject']):
    ax7.plot([1, 2], [bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'rt_no_stim'],
                      bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'rt_stim']],
             color = colors[bias_df.loc[bias_df['subject'] == subject, 'sert-cre'].values[0]], marker='o', ms=2)
ax7.set(xlabel='', xticks=[1, 2], xticklabels=['No stim.', 'Stim.'], ylabel='Median reaction time',
        yscale='log', yticks=[1], yticklabels=[1])

for i, subject in enumerate(bias_df['subject']):
    ax8.plot([1, 2], [bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'perf_no_stim'],
                      bias_df[bias_df['sert-cre'] == 1].loc[bias_df['subject'] == subject, 'perf_stim']],
             color = colors[bias_df.loc[bias_df['subject'] == subject, 'sert-cre'].values[0]], marker='o', ms=2)
ax8.set(xlabel='', xticks=[1, 2], xticklabels=['No stim.', 'Stim.'], ylabel='Performance (%)',
        ylim=[65, 80])
