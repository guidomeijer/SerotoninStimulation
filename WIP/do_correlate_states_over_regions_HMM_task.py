...
for i, this_rec in enumerate(all_rec):
    ...
    rec_region_paths = glob(join(save_path, 'HMM', 'Task', f'{N_STATES_SELECT}', 'prob_mat', f'{this_rec[:20]}*'))
    rec_region = dict()
    ...
    for r1, region1 in enumerate(rec_region.keys()):    
        for r2, region2 in enumerate(list(rec_region.keys())[r1:]):
            ...
            for tb, bin_center in enumerate(bin_centers):
                ...
                this_corr_mat = np.empty((opto_region[region1].shape[2], opto_region[region2].shape[2]))
            ...
            corr_df = pd.concat((corr_df, pd.DataFrame(data={
                'time': bin_centers, 'r_mean': corr_mean, 'opto': 1,
            ...
            for tb, bin_center in enumerate(bin_centers):
                ...
                # Correlate each state with each other state
                this_corr_mat = np.empty((no_opto_region[region1].shape[2], no_opto_region[region2].shape[2]))
            ...
            # Add to dataframe
            corr_df = pd.concat((corr_df, pd.DataFrame(data={
                'time': bin_centers, 'r_mean': corr_mean, 'opto': 0,
                'region1': region1, 'region2': region2,
                'region_pair': f'{np.sort([region1, region2])[0]}-{np.sort([region1, region2])[1]}',
                'subject': subject, 'date': date, 'probe': probe})))
            ...
            for tb, bin_center in enumerate(bin_centers):
                ...
                # Correlate each state with each other state
                this_corr_mat = np.empty((opto_probe_region[region1].shape[2], opto_probe_region[region2].shape[2]))
            ...
            # Add to dataframe
            corr_df = pd.concat((corr_df, pd.DataFrame(data={
                'time': bin_centers, 'r_mean': corr_mean, 'opto': 2,
                'region1': region1, 'region2': region2,
                'region_pair': f'{np.sort([region1, region2])[0]}-{np.sort([region1, region2])[1]}',
                'subject': subject, 'date': date, 'probe': probe})))
