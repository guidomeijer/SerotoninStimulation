...
# Get all files
all_files = glob(join(save_path, 'HMM', 'Passive', f'{N_STATES_SELECT}', 'prob_mat', '*.npy'))
...
for i, this_rec in enumerate(all_rec):
    ...
    # Save output
    corr_df.to_csv(join(save_path, f'state_correlation_{N_STATES_SELECT}_passive.csv'))
