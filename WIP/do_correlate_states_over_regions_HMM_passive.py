...
# Get all files
all_files = glob(join(save_path, 'HMM', 'PassiveEvent', f'{RANDOM_TIMES}', 'prob_mat', '*.npy'))
    ...
    for r1, region1 in enumerate(rec_region.keys()):
        for r2, region2 in enumerate(list(rec_region.keys())[r1:]):
            ...
            corr_mats = np.empty((rec_region[region1].shape[2], rec_region[region2].shape[2],
                                  bin_centers.shape[0]))
            corr_mats_null = np.empty((rec_region[region1].shape[2], rec_region[region2].shape[2],
                                       bin_centers.shape[0]))
            ...
            corr_abs, corr_abs_null = np.empty(bin_centers.shape[0]), np.empty(bin_centers.shape[0])
            ...
            for tb, bin_center in enumerate(bin_centers):
                ...
                for state1 in range(rec_region[region1].shape[2]):
                    for state2 in range(rec_region[region2].shape[2]):
                        ...
                        # Bin state probabilities
                        r1s1_null = np.mean(rec_region[region1][
                            :rec_region[region1].shape[0]//2,
                            (time_ax >= bin_center - BIN_SIZE/2) & (time_ax <= bin_center + BIN_SIZE/2),
                            state1],
                            axis=1)
                        r2s2_null = np.mean(rec_region[region2][
                            :rec_region[region2].shape[0]//2,
                            (time_ax >= bin_center - BIN_SIZE/2) & (time_ax <= bin_center + BIN_SIZE/2),
                            state2],
                            axis=1)
                ...
                corr_abs[tb] = np.mean(np.abs(corr_mats[:, :, tb]))
                corr_mean_null[tb] = np.mean(corr_mats_null[:, :, tb])
                corr_abs_null[tb] = np.mean(np.abs(corr_mats_null[:, :, tb]))
            ...
            corr_df = pd.concat((corr_df, pd.DataFrame(data={
                ...
                'r_abs': corr_abs, 'region1': region1, 'region2': region2, 'opto': 1,
                'region_pair': f'{np.sort([region1, region2])[0]}-{np.sort([region1, region2])[1]}',
                'subject': subject, 'date': date})))
            corr_df = pd.concat((corr_df, pd.DataFrame(data={
                'time': bin_centers, 'r_mean': corr_mean_null, 'r_max': corr_max_null,
                'r_min': corr_min_null, 'r_abs': corr_abs_null,
