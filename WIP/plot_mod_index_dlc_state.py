...
# Load data
state_mod_neurons = pd.read_csv(join(data_path, 'mov_pupil_state_mod.csv'))
...
all_neurons['mod_diff_mov'] = all_neurons['mod_index_active'] - all_neurons['mod_index_inactive']
all_neurons['mod_diff_pupil'] = all_neurons['mod_index_large_pupil'] - all_neurons['mod_index_small_pupil']
...
sns.swarmplot(data=all_df, y='mod_diff_mov', ax=ax3, color=colors['sert'])
...
for i in per_animal_df.index:
    ax1.plot([0, 1], [enh_df.loc[i, 'mod_index_small_pupil'], enh_df.loc[i, 'mod_index_large_pupil']],
             '-o', markersize=2, color=colors['enhanced'])
_, p = wilcoxon(enh_df['mod_index_small_pupil'], enh_df['mod_index_active'])
ax1.set(ylabel='Modulation index', xticks=[0, 1], xticklabels=['Small', 'Large'], xlabel='State',
        title=f'Enhanced neurons (p={p:.2f})', ylim=[-0.12, 0.41])

for i in per_animal_df.index:
    ax2.plot([0, 1], [supp_df.loc[i, 'mod_index_small_pupil'], supp_df.loc[i, 'mod_index_large_pupil']],
             '-o', color=colors['suppressed'])
_, p = wilcoxon(supp_df['mod_index_small_pupil'], supp_df['mod_index_large_pupil'])
ax2.set(ylabel='Modulation index', xticks=[0, 1], xticklabels=['Small', 'Large'], xlabel='State',
        title=f'Suppressed neurons (p={p:.2f})', ylim=[-0.5, 0.01])

ax3.plot([-0.5, 1], [0, 0], color='grey', ls='--')
sns.swarmplot(data=all_df, y='mod_diff_pupil', ax=ax3, color=colors['sert'])
ax3.set(ylabel='Modulation change (up-down)', yticks=[-0.4, -0.3, -0.2, -0.1, 0, 0.1])
