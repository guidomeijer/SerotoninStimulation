...
fig_path = join(f_path, 'Extra plots', 'State', 'Simultaneous Regions', f'{N_STATES_SELECT}')

# Get all files
all_files = glob(join(save_path, 'HMM', f'{N_STATES_SELECT}', 'state_mat', '*.npy'))
...
for i, this_rec in enumerate(all_rec):
    ...
    rec_region = glob(join(save_path, 'HMM', f'{N_STATES_SELECT}', 'state_mat', f'{this_rec[:20]}*'))
